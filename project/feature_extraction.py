...
class ImageFeatureExtractor(nn.Module):
    def __init__(
        ...
        encapsulates up to four models:
            self.encoder: The encoder CNN (N is the batch size)
                (N, 3, H_in, W_in) -> (N, projection_in, H_out, W_out)
            self.pooling: An adaptive average pooling layer.
                (N, projection_in, H_out, W_out) -> (N, projection_in, 1, 1)
        ...
        CNN activation map (to use as features for an attention meachanism).
        ...
        Args:
            ...
            projection_out:
                ...
                channels for the 1x1 convolution layer. Typically, this would
                supply the initial hidden state and/or features for an attention
                mechanism.
        """
        super().__init__()
    ...
    def forward(self, image):
        raise NotImplementedError(
            "Don't the extractor directly; use the " "submodels instead."
        )


class WordEmbedder(nn.Module):
    """
    Supplies an instance of nn.Embedding, along with an initialization
    function, using the padding character from the supplied tokenizer:
        embedder: (vocab_size, wordvec_dim)
        tokenizer: a fitted instance of tokenizers.BertWordPieceTokenizer.
        The padding token should be project.datasets.PAD
    
    This module does not include a nonlinearity.
    """
